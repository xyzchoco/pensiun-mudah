...
export default function Kuis({ learning }) {
  ...
  // ✅ Ambil module id dari URL (?module=2), lalu pilih KUIS MODUL ITU
  const { url } = usePage();
  const params = new URLSearchParams(url.split("?")[1] || "");
  const moduleIdParam = params.get("module");

  const targetModule = moduleIdParam
    ? modules.find((m) => String(m.id) === String(moduleIdParam))
    : modules.find((m) => m.quiz); // fallback: modul pertama yg ada kuis
  const quiz = targetModule?.quiz || null;

  // Guard: modul ini nggak punya kuis
  if (!quiz || !Array.isArray(quiz.questions) || quiz.questions.length === 0) {
    return (
      <LearningLayout>
        <Head title="Kuis - Pensiun Mudah" />
        <main className="mx-auto max-w-[720px] px-8 py-20 text-center">
          <h1 className="text-2xl font-extrabold text-[#1F1F1F]">
            Kuis belum tersedia
          </h1>
          <p className="mt-3 text-[#3D4A3E]">
            Modul ini belum punya soal kuis, atau kuis tidak ditemukan.
          </p>
          <Link
            href={`/pelatihan/${course.id}/kelas`}
            className="mt-6 inline-flex items-center gap-2 rounded-xl border-2 border-[#007A3D] px-6 py-3 font-extrabold text-[#007A3D]"
          >
            <ArrowLeft className="h-5 w-5" /> Kembali ke Pelatihan
          </Link>
        </main>
      </LearningLayout>
    );
  ...
  const handleBack = () => {
    if (currentIndex > 0) {
      ...
      router.visit(`/pelatihan/${course.id}/kelas`);
    }
  };
  ...
  const handlePrimary = () => {
    ...
    router.post(`/pelatihan/${course.id}/kuis/selesai`, {
      quiz_id: quiz.id, // ✅ sekarang = id kuis modul yang BENAR
    ...
    <LearningLayout>
      ...
      <main className="mx-auto max-w-[1072px] px-8 pb-6 pt-9">
        ...
        <div className="mt-5 flex items-center gap-2 text-base font-bold">
          <Link href="/dashboard" className="text-[#007A3D]">Beranda</Link>
          <ChevronRight className="h-4 w-4 text-[#007A3D]" />
          <Link href={firstLessonHref} className="text-[#007A3D]">Modul Saya</Link>
          <ChevronRight className="h-4 w-4 text-[#007A3D]" />
          <span className="font-medium text-[#3D4A3E]">{quiz.title || "Kuis"}</span>
        </div>
        ...
        <div className="mt-8 flex items-start justify-between gap-8">
          ...
          <div className="mt-1 w-[216px] shrink-0 rounded-xl border border-[#BFD1C0] bg-[#F0EDED] p-4">
            <div className="flex justify-between items-center text-sm font-extrabold text-[#1F1F1F]">
              ...
              <span className="text-[#007A3D]">{progressStep} dari {totalQuestions}</span>
            </div>
          ...
          <div className="mt-9 space-y-4 pl-1 md:pl-10">
            {(currentQuestion?.options || []).map((option, index) => {
              ...
              return (
                ...
                >
                  <span
                    className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-full border-2 transition ${active ? "border-[#007A3D] bg-white" : "border-[#6C7B70] bg-white"
                      }`}
                  >
                    {active && <span className="h-3 w-3 rounded-full bg-[#007A3D]" />}
                  </span>
                  <span className="w-8 shrink-0 font-extrabold text-[#007A3D]">
                    {answerLetters[index]}
                  </span>
                  <span className="text-xl leading-7 text-[#1F1F1F]">{optionText}</span>
                </button>
              );
            })}
          </div>
