...
export default function PilihJadwal({
    ...
    submitUrl = '/korporat/request-jadwal',
    midtransClientKey,
    pendingRequest = null, // { id, status, url }
    isEventPassed = false,
    ...
    const isLocked = Boolean(pendingRequest) || isEventPassed;
    ...
    const [isPaying, setIsPaying] = useState(false); // State untuk tombol pembayaran

    // Load Midtrans Snap
    useEffect(() => {
        if (!midtransClientKey) return;
        if (!window.snap) { // Cek window.snap, bukan document.querySelector
            ...
            if (window.snap && data.snap_token) {
                window.snap.pay(data.snap_token, {
                    onSuccess: async () => {
                        ...
                        const { data: finalizeData } = await axios.post(route('korporat.pembayaran.finalize', data.request_id));
                        router.visit(finalizeData.redirect);
                    },
                    onPending: () => {
                        router.visit(route('korporat.pembayaran-berhasil', data.request_id));
                    },
                    onError: () => {
                        ...
                        <div className="rounded-2xl border border-[#E4E2E1] bg-white p-6 shadow-sm lg:col-span-2">
                            <div className="flex items-start justify-between gap-4">
                                ...
                                <div className="flex items-center gap-3">
                                    <button type="button" onClick={goPrevMonth} disabled={isLocked} className="flex h-10 w-10 items-center justify-center rounded-lg border border-[#E4E2E1] text-[#006B32] transition-colors hover:bg-[#F0EDED] disabled:cursor-not-allowed disabled:opacity-60 disabled:border-[#C7CAC8] disabled:text-[#C7CAC8]" aria-label="Bulan sebelumnya">
                                    ...
                                    <button type="button" onClick={goNextMonth} disabled={isLocked} className="flex h-10 w-10 items-center justify-center rounded-lg border border-[#E4E2E1] text-[#006B32] transition-colors hover:bg-[#F0EDED] disabled:cursor-not-allowed disabled:opacity-60 disabled:border-[#C7CAC8] disabled:text-[#C7CAC8]" aria-label="Bulan berikutnya">
                                        <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                                    ...
                                    pendingRequest.status === 'menunggu_bayar' ? (
                                        <button
                                            ...
                                            onClick={async () => {
                                                ...
                                                setIsPaying(true);
                                                try {
                                                    const { data } = await axios.post(pendingRequest.snapTokenUrl);
                                                    if (data.snap_token && window.snap) {
                                                        window.snap.pay(data.snap_token, {
                                                            onSuccess: async () => {
                                                                const { data: finalizeData } = await axios.post(data.finalize_url);
                                                                router.visit(finalizeData.redirect);
                                                            },
                                                            onPending: () => {
                                                                router.visit(route('korporat.pembayaran-berhasil', pendingRequest.id));
                                                            },
                                                            onError: () => {
                                                                alert('Pembayaran gagal atau terjadi kesalahan.');
                                                                setIsPaying(false);
                                            ...
                                            disabled={isPaying}
                                            className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-[#006B32] py-3.5 font-bold text-white transition-colors hover:bg-[#005A2B] disabled:cursor-not-allowed disabled:opacity-60"
                                        >
                                            {isPaying ? 'Memproses Pembayaran...' : <><span className="text-xl">💳</span> Lanjutkan Pembayaran</>}
                                        </button>
                                    ) : (
                                        <Link
                                            href={pendingRequest.statusUrl}
                                            className="mt-4 flex w-full items-center justify-center gap-2 rounded-lg bg-[#006B32] py-3.5 font-bold text-white transition-colors hover:bg-[#005A2B]"
                                        >
