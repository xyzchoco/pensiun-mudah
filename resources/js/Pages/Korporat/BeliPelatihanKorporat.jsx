...
function buildScheduleHref(course, qty) {
    ...
    // Menggunakan slug di path, dan query params untuk data lainnya
    return `/korporat/pilih-jadwal/${course.slug || course.id}?${params.toString()}`;
}
    ...
    if (name === 'people') {
        return (
            <svg className="h-5 w-5" fill="none" stroke="currentColor" strokeWidth="2" viewBox="0 0 24 24">
                ...
                <path strokeLinecap="round" strokeLinejoin="round" d="M2 20c0-3 3-5 6-5s6 2 6 5M15 20c0-2 1-3 3-3s3 1 4 3" />
            </svg>
        );
    }
    return (
            ...
            <div className="relative flex h-40 items-center justify-center bg-gray-100 overflow-hidden">
                {course.thumbnail ? (
                    ...
                    <div className="w-full h-full bg-gradient-to-br from-[#3A6B2E] to-[#1F3D17] flex items-center justify-center">
                        <svg className="h-12 w-12 text-white/80" fill="none" stroke="currentColor" strokeWidth="1.6" viewBox="0 0 24 24">
                            <path strokeLinecap="round" strokeLinejoin="round" d="M12 21c-4-2-7-5-7-9 4 0 7 3 7 7M12 21c4-2 7-5 7-9-4 0-7 3-7 7M12 21V10" />
                        </svg>
                    </div>
                )}
                ...
                {course.category && (
                    <span
                        ...
                        style={{
                            backgroundColor: course.category.warna_bg_icon || '#C2255C',
