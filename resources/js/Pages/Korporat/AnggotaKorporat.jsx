...
export default function AnggotaKorporat({ members, totalAnggota, rataProgres, pagination }) {
    const pages = Array.from({ length: pagination.last }, (_, i) => i + 1);

    return (
        <KorporatLayout
            title="Anggota Korporat - Pensiun Mudah"
            activeNav="anggota"
                    ...
                    {pagination.total > 0 && (
                        <div className="flex flex-col gap-4 border-t border-[#E4E2E1] px-6 py-5 sm:flex-row sm:items-center sm:justify-between">
                            ...
                            <div className="flex items-center gap-2">
                                <Link
                                    href={route('korporat.anggota', { page: pagination.current - 1 })}
                                ...
                                {pages.map((page) => (
                                    <Link
                                        key={page}
                                        href={route('korporat.anggota', { page })}
                                        only={['members', 'pagination']}
                                        preserveScroll
                                        className={
