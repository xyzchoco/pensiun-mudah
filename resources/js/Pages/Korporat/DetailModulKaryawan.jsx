                ...
                <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6 lg:px-8">
                    ...
                    <div className="relative overflow-hidden rounded-t-2xl bg-[#006B32] p-8 text-white shadow-sm sm:p-10">
                        <div className="relative z-10">
                            ...
                            <h1 className="mt-3 text-3xl font-extrabold sm:text-4xl">{moduleName}</h1>
                        ...
                        <svg className="pointer-events-none absolute -right-4 top-1/2 h-56 w-56 -translate-y-1/2 text-white/10" fill="currentColor" viewBox="0 0 24 24">
                    ...
                    <div className="rounded-b-2xl border-x border-b border-[#E4E2E1] bg-white shadow-sm">
                        {/* Table Header */}
                        <div className="hidden grid-cols-2 gap-4 border-b border-[#E4E2E1] bg-[#F9F9F9] px-8 py-4 text-sm font-bold text-[#3D4A3E] sm:grid">
                            <span className="text-center sm:text-left">Nama Karyawan</span>
                            <span className="text-center sm:text-left">Progres Belajar</span>
                        </div>
                        ...
                        <div className="divide-y divide-[#E4E2E1]">
                            {employees.data.length > 0 ? (
                                employees.data.map((employee) => {
                                    ...
                                    const barColor = isHighProgress ? 'bg-[#006B32]' : 'bg-[#A8632A]';
                                    const textColor = isHighProgress ? 'text-[#006B32]' : 'text-[#A8632A]';

                                    return (
                                        <div key={employee.id} className="grid grid-cols-1 gap-6 px-8 py-5 sm:grid-cols-2 sm:items-center transition-colors hover:bg-[#FBF9F8]">
                                            {/* Kolom Nama & Email */}
                                            <div className="flex items-center gap-4">
                                                ...
                                                <div className="min-w-0">
                                                    <p className="font-bold text-[#1B1C1C] truncate">{employee.name}</p>
                                                    <p className="text-xs text-[#6B7280] truncate">{employee.email}</p>
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex items-center gap-4">
                                                ...
                                                <span className={`w-12 shrink-0 text-right text-sm font-bold ${textColor}`}>
                                                    {employee.progress}%
                                                </span>
                                            </div>
                                        </div>
                                ...
                                <div className="py-10 text-center">
                                    <p className="text-sm font-semibold text-[#6B7280]">Belum ada anggota yang terdaftar di pelatihan ini.</p>
                                </div>
                            )}
                        </div>
                        ...
                        {employees.total > 0 && (
                            <div className="flex flex-col items-center justify-between gap-4 border-t border-[#E4E2E1] bg-[#F9F9F9] px-8 py-4 sm:flex-row">
                                <p className="text-sm font-bold text-[#3D4A3E]">
                                    Menampilkan {employees.from}-{employees.to} dari {employees.total} anggota
                                </p>
